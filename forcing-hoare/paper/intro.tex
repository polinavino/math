...
step-indexed semantics, at the pointwise product to demonstrate
genericity, and at the lexicographic $\omega^2$ of transfinite
...
$\omega$ (iterated later exhausts at $\omega$, not at $\omega^2$).
Löb's rule, in the abstract setting, is proved by
\texttt{well\_founded\_ind} on the strict refinement relation, making
precise the equivalence between the internal Löb rule and
...
(Appendix~\ref{sec:bridge}).  We write the JTS translation as a
standalone syntactic \texttt{Fixpoint} on a deep-embedded language
with $\triangleright$ and prove a factorization theorem: over an
\emph{arbitrary} forcing structure it coincides pointwise with the
framework's semantics (\texttt{jts\_is\_interp}), and L\"ob's rule
holds of the translated formulas (\texttt{jts\_Lob}).  The
\emph{term-level} translation of full CIC remains future work.
