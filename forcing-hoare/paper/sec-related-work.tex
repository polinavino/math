...
\cite{forcing-coq-plugin}, which implements the type-level translation
generically and includes, as its main demonstrative example, a
step-indexed model of pure $\lambda$-calculus with a $\mathtt{fixp}$
operator and equational lemmas about the $Y$-combinator.  The plugin
is a translation tool: it does not develop a Hoare logic, a
weakest-precondition predicate, an operational semantics, or any
program-logic soundness proof.  Its \texttt{NatForcing} instantiation
produces a structure of condition-indexed sheaves that, up to
definitional details, has the shape of our \texttt{iProp};
Section~\ref{sec:bridge} discusses the relationship in detail.  What
the JTS line provides and we do not is a full translation of CIC
including dependent types; formalizing the propositional fragment of
that translation against our \texttt{iProp} is the natural future
direction.
...
Almost every ingredient is, individually, well-known to specialists
in some adjacent community: the topos-of-trees perspective is
familiar in synthetic guarded domain theory; the \textsf{GL} model
theory is familiar to modal logicians; Cohen's ramified forcing is
familiar to set theorists; the JTS translation is familiar to type
theorists; the predicative reading of ramification is familiar to
foundations researchers.  What does not appear in the literature, as
a single mechanized account, is a step-indexed Hoare logic that
...
forcing-style re-presentation that makes both stateable.  The
contribution is the diagnostic, the counterexample, and the framework
that supports them; the integration with the surrounding literatures
is what makes the paper cohere.  We omit a survey of classical Hoare
logic and separation logic, referring the reader to standard sources
