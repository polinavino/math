...
Proof.
  ...
  apply ientails_trans with
    (iLater (recursive_spec (EFix body) S)
       → recursive_spec (EFix body) S).
  - intros n _ m Hmn Hlater. destruct m as [|m'].
    + intros v Hvalue HS. simpl. trivial.
    + simpl in Hlater. intros v Hvalue HS.
      (* fix-unfolding via wp_pure_step_at; the recursive *)
      (* obligation is discharged from Hlater at m'.      *)
...
manufacture that recursion from outside, while the internal Löb rule
has it built in.  Section~\ref{sec:imp-vs-lam} makes this structural
observation precise.
