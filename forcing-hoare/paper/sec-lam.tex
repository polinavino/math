...
For IMP, step-indexing was inessential: the index was a presentation
convenience.  We now move to a language where it is not: an untyped
$\lambda$-calculus with a fix-point operator.
Two things change.  First, the wp predicate must explicitly encode
\emph{safety}, since the language has stuck terms (e.g.\
$\mathtt{EApp}\, (\mathtt{EInt}\, 5)\, (\mathtt{EInt}\, 3)$).  Second,
the recursion introduced by \texttt{fix} turns the soundness predicate
...
satisfying the spec, then the fix-point itself satisfies the spec.  The
...
contained in \texttt{iLob}, written once in Section~\ref{sec:prop}.
...
recursive reasoning principle.  A meta-level induction would supply
that recursion from outside the predicate. The internal Löb rule
already contains it.  Section~\ref{sec:imp-vs-lam} makes this
structural observation precise.
