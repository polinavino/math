...
\emph{available}, meaning it does not become impossible, but it has to
manufacture the recursive reasoning from outside the predicate it is
proving, instead of finding it already inside.  If the soundness
...
with predicate $S$ falls on the hard side, and Löb earns its keep
there.
...
$\triangleright \Phi$ of Löb's rule supplies.  The two cases of this
paper fall cleanly on the two sides of this distinction. Whether
the characterization can be sharpened to a syntactic check on
arbitrary iProp definitions is open.
...
rule is what plays to that structure. It is, at bottom,
