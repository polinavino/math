...
product-rule half of Theorem~\ref{thm:cox}.  The main development
does not depend on it.  The target shape is Acz\'el's associativity
...
strictly monotone $G$.  \textsf{mathlib} offers no route.  It has no
...
from our first bundle, and the additivity proof needs it.
...
refinement sandwich $2N + 1 \le N' \le 2N + 2$, where $N$ and $N'$
are the counts at consecutive refinement levels $n$ and $n+1$, makes
them monotone and Cauchy, and the generator is their limit,
...
\mathrm{dcount}\,y + 3$) against a vanishing $2^{-n}$ error term.  And
...
$x < y$ contributes a full extra block of $2^{n-M}$ copies, $M$
being the refinement level of the inserted root.
...
$\bar G = \exp(-g)$ transports it to the $[0,1]$-valued conjunctive
...
(\texttt{hasOrderedGenerator\_\allowbreak logSumExp}).  Of H\"older's classical
