...
%% TODO (planned expansion of this paragraph; absorbed the abandoned
%% sec-classicality plan -- do NOT add a separate section, bullets
%% 1-3 of that plan are already the two paragraphs below and the
%% lower-reals contrast is in sec-related):
%%  - name the classical mathlib results consumed and their strength
%%    (prime ideal separation ~ BPI/choice, classical ENNReal/real
%%    analysis in the Aczel development), and state the rule for
%%    consuming them without contaminating object-level claims
%%  - collect the non-vacuity guards in one visible place
%%    (nonempty_coxModel, nonempty_scale, the halting guard); this
%%    claim was cut from the intro contributions and currently
%%    appears only piecemeal
...
where the needed case split is a constructor check, witnessed by a
computable \texttt{Decidable} instance rather than the classical
one, and the maximal-point peel behind the general decomposition
was arranged to make no excluded-middle case split in its script.

The division also bites in a place the theory itself predicts.
Every valuation in the development whose \emph{definition} requires
classical logic is an indicator: the point valuations, the
prime-ideal indicators behind \texttt{sharp\_iff\_point}, the
$\top$-indicator, and the halting weights.  An indicator into
$[0,\infty]$ must decide membership, so the sharp states are exactly
the objects a constructive meta-theory could not build.  Likewise
the hard direction of the R3 hinge rests on prime-ideal separation,
a choice principle.  The formalization thus enacts its own thesis
one level up: certainty and points are classical resources, inside
the theory and above it.
