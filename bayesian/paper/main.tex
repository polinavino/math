...
\begin{abstract}
Kolmogorov's probability is defined within a classical logic framework.
Classical logic can also be derived from its axioms when 
reasoning about events that are certain. Quantum
mechanics has showed that this logical substrate is just 
...
$v(\neg A) = 1 - v(A)$ holds for every valuation exactly when the
algebra satisfies excluded middle, and (2) Dempster conditioning agrees
with Bayesian conditioning exactly when the evidence does.
...
removing it from a semidecidable event forces the halting problem
to be solvable.  We further prove a dichotomy delimiting the probability-monad
question, show that the naive transcription of Cox's theorem is
