...
finite frame, not just lower sets
(\texttt{Valuation.\allowbreak exists\_pmf\_presentation}).  Let us define the \emph{mass} of a
...
\begin{theorem}[finite representation,
  \texttt{Valuation.eq\_\allowbreak sum\_mass}, \texttt{Valuation.sum\_mass}]
...
\begin{theorem}[mixture characterization,
  \texttt{Valuation.eq\_\allowbreak mix\_\allowbreak deltaPoint}]
\label{thm:mixture}
...
The $\top$-indicator of Theorem~\ref{thm:obstruction} witnesses this:
it is not Scott-continuous (\texttt{topIndicator\_not\_scott}).  Scott
continuity is exactly what restores the representation:

\begin{theorem}[recovery, \texttt{eq\_tsum\_mass\_of\_scott},
  generalized by \texttt{Valuation.isPurelyAtomic\_of\_scott}]
\label{thm:recovery}
