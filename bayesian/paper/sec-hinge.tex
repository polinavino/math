...
theory recovers the \emph{points} of the locale, and the refinement
to points in the spatial sense is exact as well.  A prime ideal is
...
$v$ is sharp and Scott-continuous iff $v$ is the complement-indicator
of a completely prime ideal.
\end{theorem}

\noindent
So the finitely prime points of \texttt{sharp\_iff\_point} sharpen to
the completely prime points exactly where Scott continuity is added,
recovering points of the locale in the sense used
by~\cite{vickers2008,coquand-spitters2009}.  The hinge and the two
correspondences together are the paper's static answer to ``where
does classicality live'': in one axiom, whose semantic shadow is
point-hood.
