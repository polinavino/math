...
recover the \emph{shape} of Dempster--Shafer (DS)
...
never relaxed.  What changes is only the join used to state the
...
\subsection{Dynamics: the conditioning hinge}
...
\[
  ...
  \frac{v(a \sqcup b^{\mathsf{c}}) - v(b^{\mathsf{c}})}
       {1 - v(b^{\mathsf{c}})}.
\]
Both are again valuations (\texttt{dempsterCond}).  Their modularity
...
vanishes at \emph{every} $a$, in turn, depends on a single number,
...
The static hinge is an equation in the frame itself, between
$b \sqcup b^{\mathsf{c}}$ and $\top$, with no valuation involved.
The dynamic hinge tests the same proposition through $v$:
