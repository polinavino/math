...
Classical logic underlies Kolmogorov's probability~\cite{kolmogorov1933}.
The axioms of this probability
specify an arithmetic of chance (non-negativity, normalization,
$\sigma$-additivity). Beneath that arithmetic sits a logic: the events
form a Boolean $\sigma$-algebra, closed under complement and
satisfying excluded middle.  The complement rule,
$P(A^{c}) = 1 - P(A)$, is where the two layers touch.  It is an
...
complement.  In a Heyting algebra the pseudocomplement $\neg a$
satisfies
...
measure of ignorance.  The theory that results recognizably has the same
structure as the Dempster--Shafer belief functions, but derived from
...
$[\,v(A),\, 1 - v(\neg A)\,]$ of width $\slack(A)$, so point
...
query being answered: the value of an event $A$ is $P(\mathrm{int}\,A)$, i.e. the
probability of its verifiable part.
...
$\neg\neg a \sqcup \neg a = \top$, is one such case. Using
...
\begin{enumerate}
\item \textbf{The two characterizations}
...
\item \textbf{Cox's theorem, dissected} (Section~\ref{sec:cox},
...
\item \textbf{Mechanization}.
  Everything above is mechanized in Lean~4~\cite{moura-ullrich2021}
  using \textsf{mathlib}~\cite{mathlib2020}, and is (to our knowledge) the first
