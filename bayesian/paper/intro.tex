...
algebras~\cite{birkhoff-vonneumann1936}, and Gleason's
theorem~\cite{gleason1957} plays the
role there that this paper's representation theorems play here:
it identifies the states compatible with the event logic.  Nobody
...
Our question is what happens at a different value of the same 
parameter, motivated by the logic used to reason about computation:
...
modular map $v : \Omega \to \ENNReal$.  Modularity is inclusion--exclusion,
...
i.e. the probability of its verifiable part. 
...
informal case split over $\{A, \neg A\}$ silently decides an
...
Section~\ref{sec:engineering} justifies each appearance of
classical logic in this work, including at the meta-theory level.
...
\item \textbf{Mechanization} (Section~\ref{sec:engineering}).
